...
impl Site {
    pub fn construct_header(&mut self, page: &Page) -> String {
        ...
        let home_link = self.construct_nav_wrapped_link(&home_nav_item, &page.foundation);
        
        let nav_links = self.construct_nav_links(&nav_items, &page.foundation);
        
        let call_us_link = self.construct_nav_wrapped_link(&call_us_item, &page.foundation);
