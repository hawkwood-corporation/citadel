...
impl<T> Site<T> {
    pub fn construct_homepage(&mut self, page: &mut Page<T>) {
        ...
        page.foundation.metadescription = Some("Strategic revenue engineering for serious businesses. We deliver 10:1+ ROI through sophisticated Google Ads systems that perform under pressure.".to_owned());

        let head = self.construct_head(page);
        let header = self.construct_header(page);
        let hero = self.construct_homepage_hero();
        ...
        page.foundation.content = Some(content);
    }
}
